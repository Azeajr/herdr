...
Experimental Browser panes come back as Browser panes on the page they were last on, in their old place in the layout. The browser process itself is not preserved: Herdr stops it on shutdown and starts a fresh one on the saved url, so page state below the url — scroll position, form input, in-memory cookies — is lost. If `[experimental].kitty_graphics` has been turned off since the session was saved, or the session holds more Browser panes than the four-pane limit allows, the extra panes come back in place showing why they did not start, and `r` retries them.
