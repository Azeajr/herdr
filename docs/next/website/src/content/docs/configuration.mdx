...
Enable it only when testing terminal image behavior. Experimental Browser panes draw through the same overlay, so `herdr browser open` also requires this setting and a client terminal that speaks the Kitty graphics protocol.
