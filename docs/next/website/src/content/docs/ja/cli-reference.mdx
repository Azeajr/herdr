...
herdr peer add <name> (--socket PATH | --ssh DESTINATION) [--peer-session NAME] [--json]
herdr peer remove <name> [--json]
herdr peer open <target> [--peer NAME] [--label TEXT] [--focus] [--no-focus] [--takeover]
```

ピアとは、このサーバーが自分のワークスペースと並べて表示する別の Herdr サーバーです。このマシン上の API ソケットのパス、または SSH の宛先で指定します:
...
ピアのセッションを指定するフラグは `--peer-session` です。`--session` は、すべての `herdr` コマンドが自分のどのサーバーに対して話すかを選ぶために既に使われているためです。

`add`、`remove`、`list` はいずれもピアの一覧全体を返し、表として出力します。`--json` を付けると生のレスポンスが得られ、各ピアのバージョン、プロトコル、報告されたワークスペースも含まれます。

`peer open` は `peer list` から得たワークスペース id、ペイン id、ターミナル id を受け取り、それを表示するローカルのワークスペースを開きます。`peer list` の id はピアのインスタンス id で名前空間化されており、それだけでピアを特定できます。`--peer` を渡すのは、ピアローカルの素の id のときだけです。

ピアが背後にあるペインで何ができて何ができないかは[永続化とリモートアクセス](/ja/docs/persistence-remote/)を参照してください。
...
`agent explain` は、スクリーン検出が使うのと同じ下部バッファの検出スナップショットの分類を実行中のサーバーに依頼します。そのためライブの出力はサーバーのアクティブなマニフェストキャッシュを反映します。これは `agent.explain` ソケットメソッドを使うので、Herdr のアップグレード後にライブ explain を使う前に、サーバーを再起動するか更新済みサーバーへハンドオフしてください。保存済みフィクスチャをローカルで説明するには `--file PATH --agent LABEL` を使います。デフォルトの出力には、エージェント、最終状態、マニフェストのソースとバージョン、リージョンの証拠付きでマッチしたルール、そしてフォールバック・スキップ・警告の理由が表示されます。ピアが背後にあるペインでは、実際にルールを実行したピアが説明を返し、マニフェスト行の上の `peer:` 行がそれを示します。その下に並ぶ内容はすべてピア側のインストールの話です。`--verbose` を付けると、可視の証拠フラグ、キャッシュされたリモートバージョン、ローカルオーバーライドのシャドーイング、リモート更新状況、マッチャーとリージョンの証拠付きの評価済みルール全リストが表示されます。issue の報告やテストには `--json` を付けてください。
