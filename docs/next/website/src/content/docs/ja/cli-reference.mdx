...
ブラウザペインは、実際の Chromium のページをターミナルと並ぶ Herdr のペインの中に描画します。
...
ブラウザペインは実験的機能で、2 つの要件があります。描画にはペイングラフィックスのオーバーレイを使うため、`[experimental].kitty_graphics = true` と、Kitty グラフィックスプロトコルに対応したクライアントターミナルが必要です。これがない場合、`browser open` は `feature_disabled` で失敗します。また、外部の [`agent-browser`](https://www.npmjs.com/package/agent-browser) CLI を通じてヘッドレス Chrome を操作するため、これが `PATH` 上に必要です。

`browser open` は `--pane`(省略時はフォーカス中のペイン)から新しいペインを分割し、`.result.pane` で返します。URL を省略するとペインは `about:blank` で開きます。ブラウザペインはそれぞれが完全なブラウザとポーリングスレッドを伴うため、同時に開けるのは最大 4 つです。5 つ目を開こうとする `browser open` は起動せずに失敗します。

`browser info` はペインが表示しているページを報告します。
...
ページのホスト名はペインのラベルに、ページタイトルはペインのターミナルタイトルになります。そのため `pane list` やサイドバーでも、別途呼び出すことなくブラウザペインの表示内容が分かります。

ブラウザセッションが停止した場合、Herdr は数回まで再起動を試み、その後はペインをそのまま残して失敗内容を表示します。そのペインで `r` を押すと同じページで再試行します。ペインはその間もレイアウト上の位置を保持するため、復帰したセッションは元の場所に戻ります。
