...
`herdr status server` includes an `instance:` line with the server's instance
id. That id is stable across restarts and is how another Herdr server names this
one when [federating with it as a peer](/docs/persistence-remote/).
...
```bash
herdr peer connect <destination> [--name NAME] [--peer-session NAME] [--new] [--yes] [--skip-ssh-check]
herdr peer setup-ssh <destination> [--yes]
herdr peer list [--json]
herdr peer add [name] (--socket PATH | --ssh DESTINATION) [--peer-session NAME] [--yes] [--skip-ssh-check] [--json]
herdr peer remove <name> [--json]
herdr peer open <target> [--peer NAME] [--label TEXT] [--focus] [--no-focus] [--takeover]
```

`peer connect` is the whole path in one command: it makes sure SSH can reach the destination without a prompt, makes sure a matching Herdr is installed there, adds the peer, waits for it to connect, and opens a workspace on it — attaching to the one the peer already has focused, or creating one with `--new`. Anything interactive it needs is asked in your terminal, because the server that dials peers has none. `peer setup-ssh` runs just the SSH half, for fixing a peer that failed to authenticate.

A peer is another Herdr server whose workspaces this server shows beside its own. Name it by an API socket path on this machine, or by an SSH destination:

```bash
herdr peer add beta --socket ~/.config/herdr/sessions/beta/herdr.sock
herdr peer add workbox --ssh workbox --peer-session agents
```

The peer's session flag is `--peer-session`, because `--session` already chooses which of your own servers every `herdr` command talks to. The name is optional for an SSH peer and defaults to the destination's host. `--yes` accepts the SSH key and remote install prompts; `--skip-ssh-check` skips both checks entirely.

`add`, `remove`, and `list` all answer with the whole peer list and print it as a table; `--json` gives the raw response, which also carries each peer's version, protocol, and reported workspaces.

`peer open` takes a workspace, pane, or terminal id from `peer list` and opens a local workspace viewing it. Ids from `peer list` are namespaced with the peer's instance id, which identifies the peer on its own; pass `--peer` only for a bare peer-local id.

See [Persistence and remote access](/docs/persistence-remote/) for what a peer-backed pane can and cannot do.
