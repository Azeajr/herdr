...
请求可以包含 `if_instance_id`。如果当前实例 ID 不匹配,服务器会以 `server_replaced` 拒绝请求。联邦功能使用此前置条件,防止从一台服务器获得的 ID 操作替代它的另一台服务器。
...
| 对等 | `peer.add`、`peer.remove`、`peer.list`、`peer.workspace.open`、`peer.terminal.open` |
...
读取由对等支撑的面板时,由持有屏幕的对等作答,并以相同的结构返回,但使用的是你自己的面板、标签页和工作区 id。当对等不可达时,读取会以标明对等连接状态的 `unavailable` 失败,而不是返回一屏空白。

## 对等服务器

对等让一台 Herdr 服务器可以显示另一台 Herdr 服务器的工作区。关于对等是什么、由对等支撑的面板如何表现,见[持久化与远程访问](/zh-cn/docs/persistence-remote/);不写 JSON 而使用同样这些方法,见 [CLI 参考](/zh-cn/docs/cli-reference/)中的 `herdr peer`。
...
`peer.add`、`peer.remove` 和 `peer.list` 都返回完整的对等列表。每个条目带有 `connection`(`connecting`、`connected`、`reconnecting`、`error`)、重试中或放弃后的 `attempt` 与 `error`、对等的 `instance_id`、`version`、`protocol`、`stale` 标志,以及它最后一次报告的 `workspaces`。`reconnecting` 表示仍在重试,`error` 表示已经停止。

`peer.workspace.open` 接受工作区、面板或终端 id,并打开一个查看它的本地工作区。`peer.terminal.open` 只打开一个由对等支撑的终端,不在其外层创建工作区。两者都接受用对等实例 id 加了命名空间的 id,即 `peer.list` 返回的形式;只有传对等本地的裸 id 时才需要 `name`。对同一目标打开两次会返回你已有的视图。

对带命名空间的工作区 id 执行 `workspace.rename` 和 `workspace.close` 会转发给所属的对等。对同样的 id 执行 `workspace.focus` 则是本地操作:它会打开或切换到你对该工作区的视图。

`PaneInfo` 会在查看对等终端的面板上报告 `peer`,指明是哪个对等。在提供任何需要该面板自身屏幕的功能(例如选择或搜索)之前,先读取它。`PaneInfo` 上的其余字段对这类面板同样会被填充:`cwd`、`foreground_cwd`、`terminal_title`、`agent` 和 `agent_status` 都来自拥有屏幕的那个对等,因此对等的智能体也会出现在 `agent.list` 中,并且可以像本地智能体一样用 `agent.wait` 等待。这些字段大约在 1.5 秒内刷新而非立即刷新,连接断开时会保留最后的值而不是清空。

正因为会保留,在信任这些值之前先读 `peer_view`。由对等支撑的面板一定有它,本地面板则没有。它的 `state` 是以下之一:

- `connected` — 对等正在推送该面板,其余字段也都是最新的。
- `reconnecting` — 连接断开,正在重新打开。面板仍显示对等的最后一帧;若已知原因,`reason` 会带上最近一次失败。
- `disconnected` — 已放弃重连,`reason` 说明原因,在关闭面板并重新打开视图之前不会有任何变化。该状态用于对等不可达、或对等已被另一台服务器替换的情况;当对等报告面板本身已不存在时,视图会直接关闭,而不会进入此状态。

`state` 每次变化都会发出 `pane.updated` 事件,客户端无需轮询即可跟踪视图的存活状态。`state` 未变而只有 `reason` 变化时不会为此单独发事件 —— `pane.updated` 也会因其他原因发出,并携带当时 `peer_view` 的内容。

对由对等支撑的面板执行 `agent.read` 会像 `pane.read` 一样转发给对等。`agent.explain` 同样如此:决定该面板状态的规则是在对等上运行的,针对的是本服务器并不拥有的屏幕。转发回来的解释会多出 `peer` 和 `peer_pane_id` 两个字段,指明它来自哪里——其余每个字段,包括清单来源、清单版本和规则 id,描述的都是对等那一侧的安装,而不是本机。对等的拒绝会原样透传,并在消息前加上对等名称;对等不可达时返回 `unavailable` 并说明对等的状态,而不是由本服务器编造一份解释。

反方向是 `owner_instance_id` 和 `owner_attached`,这是一台服务器就"另一台服务器让它创建的面板"所作的报告 —— `owner_attached: false` 表示请求该面板的实例已经消失,而面板仍在这里运行。
