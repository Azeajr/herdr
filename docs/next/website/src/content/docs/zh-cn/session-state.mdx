...
实验性浏览器窗格会以浏览器窗格的形式回到布局中原来的位置,并停留在它们最后所在的页面。浏览器进程本身不会被保留:Herdr 在关闭时停止它,并在保存的 url 上启动一个新的,因此 url 之下的页面状态(滚动位置、表单输入、仅存于内存的 Cookie)会丢失。如果会话保存之后关闭了 `[experimental].kitty_graphics`,或会话中的浏览器窗格数量超过四个的上限,超出的窗格会在原地回来并显示未能启动的原因,按 `r` 可以重试。
