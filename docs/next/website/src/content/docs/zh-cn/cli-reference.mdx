...
对等是另一台 Herdr 服务器，本服务器会把它的工作区显示在自己的工作区旁边。用本机上的 API 套接字路径，或用 SSH 目标来指定它：
...
指定对等会话的参数是 `--peer-session`，因为 `--session` 已经用来选择每个 `herdr` 命令发往你自己的哪台服务器。

`add`、`remove`、`list` 都会返回完整的对等列表并以表格打印；`--json` 给出原始响应，其中还包含每个对等的版本、协议和已报告的工作区。

`peer open` 接受来自 `peer list` 的工作区 id、窗格 id 或终端 id，并打开一个查看它的本地工作区。`peer list` 返回的 id 已用对等的实例 id 加了命名空间，足以标识对等；只有对等本地的裸 id 才需要传 `--peer`。

由对等支撑的窗格能做什么、不能做什么，见[持久化与远程访问](/zh-cn/docs/persistence-remote/)。
