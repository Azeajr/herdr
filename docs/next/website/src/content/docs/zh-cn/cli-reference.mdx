...
浏览器窗格在 Herdr 窗格中渲染真实的 Chromium 页面,与你的终端并排显示。
...
浏览器窗格是实验性功能,有两个前提。它们通过窗格图形叠加层绘制,因此需要 `[experimental].kitty_graphics = true` 以及支持 Kitty 图形协议的客户端终端;否则 `browser open` 会以 `feature_disabled` 失败。它们还通过外部的 [`agent-browser`](https://www.npmjs.com/package/agent-browser) CLI 驱动无头 Chrome,该命令必须在 `PATH` 中。

`browser open` 从 `--pane`(省略时为聚焦窗格)分割出一个新窗格,并在 `.result.pane` 中返回它。不带 URL 时窗格打开 `about:blank`。由于每个浏览器窗格都是一个完整的浏览器加一个轮询线程,同时最多只能打开四个;再次 `browser open` 会直接失败,而不会启动第五个。

`browser info` 报告窗格当前所在的页面:
...
页面的主机名会成为窗格标签,页面标题会成为窗格的终端标题,因此 `pane list` 和侧边栏无需额外调用即可显示浏览器窗格正在显示的内容。

如果浏览器会话终止,Herdr 会重启若干次,之后保留该窗格并显示失败信息。在该窗格中按 `r` 可在同一页面上重试。整个过程中窗格都保留其布局位置,因此恢复后的会话会回到原处。
